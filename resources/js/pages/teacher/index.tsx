...
export default function TeacherIndex() {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    if (isEdit && form.id) {
      router.put(`/teachers/${form.id}`, payload, opts);
    } else {
      router.post('/teachers', payload, opts);
    }
  };
  ...
  const handleDelete = (id: number) => {
    if (window.confirm('Da li ste sigurni da želite obrisati ovog profesora?')) {
      router.delete(`/teachers/${id}`, {
        preserveScroll: true,
        onSuccess: () => router.reload({ only: ['teachers'] }),
      });
    }
  };
  ...
  return (
    <AppLayout breadcrumbs={breadcrumbs}>
          ...
          <table className="min-w-full border text-sm rounded-lg">
            <thead className="bg-gray-100 dark:bg-neutral-800">
              <tr>
                ...
                <th className="px-4 py-2 text-left font-semibold">Predmet</th>
      ...
      <Dialog open={open} onOpenChange={setOpen}>
        <DialogContent>
          <DialogHeader>
            <DialogTitle>{isEdit ? 'Promjeni profesora' : 'Dodaj profesora'}</DialogTitle>
          </DialogHeader>
